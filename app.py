...
from streak import calculate_streak, dates_from_rows
...
def enrich_habit(db, habit_row, today: str) -> dict:
    completions = db.execute(
        "SELECT completion_date FROM habit_completions WHERE habit_id = ?",
        (habit_row["id"],),
    ).fetchall()
    completion_dates = {row["completion_date"] for row in completions}
    return {
        "id": habit_row["id"],
        "name": habit_row["name"],
        "created_at": habit_row["created_at"],
        "streak": calculate_streak(dates_from_rows(completions)),
